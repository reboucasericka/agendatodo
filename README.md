...
- Bloco *upcoming* (próximas 8 reuniões não concluídas) disponibilizado ao frontend.

### Autenticação e contas

- Registo, login, logout, recuperação e reposição de password.
- Verificação de email e confirmação de password.
- **Two-Factor Authentication (2FA)** via Fortify, com ativação por QR code (Google Authenticator), confirmação por código, *recovery codes* e desativação (`TwoFactorChallenge.vue` + secção 2FA em `settings/Profile.vue`).
...
- A página `/breve/{feature}` serve como *placeholder* genérico (`ComingSoon.vue`) para secções ainda não implementadas, com ligações rápidas para Tarefas, lista semanal e Reuniões.
